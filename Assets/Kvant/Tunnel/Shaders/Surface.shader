//
// Opaque surface shader for Tunnel
//
// Vertex format:
// position     = not in use
...
Shader "Kvant/Tunnel/Surface" {
    ...
    SubShader {
        ...
        struct Input {
        #if _ALBEDOMAP || _NORMALMAP || _OCCLUSIONMAP || _EMISSIONMAP
            float3 localCoord;
            float3 localNormal;
        #else
            half dummy;
        ...
        #if _NORMALMAP
            v.tangent = float4(normalize(cross(v.normal, float3(0, 0, 1))), 1);
        #endif
        ...
        #if _ALBEDOMAP || _NORMALMAP || _OCCLUSIONMAP || _EMISSIONMAP
            // calculate blend for triplanar mapping
        ...
        #if _EMISSIONMAP
            // Emissive map
            half ex = tex2D(_EmissionMap, pmx).g * blend.x;
            half ey = tex2D(_EmissionMap, pmy).g * blend.y;
            half ez = tex2D(_EmissionMap, pmz).g * blend.z;
            o.Emission = (ex + ey + ez) * _Emissive * _EmissionStr;
        #endif
    ...
    CustomEditor "Kvant.TunnelMaterialEditor"
}
